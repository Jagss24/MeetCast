...
const Room = () => {
  ...
  const [userisAlreadyinWaitingList, setuserisAlreadyinWaitingList] =
    useState(false);
  ...
  const { isSuccess: isRemoveMemberSuccess, mutate: removeMemberMutate } =
    useMutation({
  ...
  const handleIsSpeaker = (userId) => {
    const isUserSpeaker = room?.speakers.find(
  ...
  useEffect(() => {
    if (room?.id && user?.id) {
      setuserisAlreadyinWaitingList(
        room?.waitingList?.some((eachUser) => eachUser?._id === user?.id)
      );
      setuserisInMemberList(
        room?.memberList?.some((eachUser) => eachUser?._id === user?.id)
      );
      setuserisInRemovedList(
        room?.removedList?.some((eachUser) => eachUser?._id === user?.id)
      );
    }
  }, [room, user]);
  ...
  return isLoading ? (
    ...
    <RoomConatiner
        ...
        <>
          ...
          <UserContainers
            isCenter={
              ...
              user?.id !== room?.ownerId?._id
            }>
            <SpeakerContainer>
              <h3>Speakers</h3>
              <div>
                {room?.speakers.map((speaker) => (
                  <EachSpeaker key={speaker?._id}>
                    <div
                ...
                <AllowContainer>
                  ...
                  <div>
                    {room?.waitingList?.length ? (
                      room?.waitingList.map((eachUser) => (
                        <EachSpeaker key={eachUser?._id}>
                          ...
                          <div className='buttons'>
                            <button
                              onClick={() =>
                                handleAddMemberToRoom(roomId, eachUser?._id)
                              }>
                              Accept
                              {isAddMemberPending && (
                      ...
                      <div>No user is present in the waiting list</div>
                    )}
                  </div>
                </AllowContainer>
              )}
          ...
          {userisInRemovedList && !isSuccess && (
            <DeclinedText>User has declined your request</DeclinedText>
          )}
          {userisAlreadyinWaitingList && (
            <SuccessText>
              Your request has already been sent. Wait for the Host to accept
              it.
            </SuccessText>
          )}
          ...
          <ButtonConatiners>
            ...
            userisInMemberList ? (
              ...
              <button
                ...
                {(isLoading || !roomId || !user?.id || isPending) && (
                  <span>
                    <Spinner width={15} height={15} />{' '}
                  </span>
                )}
      ...
      {roomType === 'podcast' && roomId && user?.id && (
        <Podcast
          user={user}
          roomId={roomId}
