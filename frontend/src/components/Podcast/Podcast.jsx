...
const Podcast = ({
  ...
  const {
    ...
    sendMessage,
    clientMessages,
  } = usePodCast({ roomId, user, isSpeaker, isOwner, showToastFunc });
  return (
    <PodContainer>
      <TopicStyle>
        <div className='icon'>
      ...
      <AudioAndChatContainer fullScreen={chat || showParticipants}>
        <OptionsContainer>
          <EachOption onClick={() => openChatorParitciPant('chat')}>
            <MdOutlineMessage
              size={'70%'}
              style={{ color: chat ? '#B0B0B0' : 'var(--button-color)' }}
            />
          </EachOption>
          <EachOption onClick={() => openChatorParitciPant('showParticipants')}>
            <FaUsers
              size={'70%'}
              style={{
                color: showParticipants ? '#B0B0B0' : 'var(--button-color)',
              }}
            />
          </EachOption>
        </OptionsContainer>
        <AudioConatiner>
          {clients.map((client, id) => (
            <AudioElement key={id}>
              ...
              <AvtarContainer
                audioElem={true}
                isUserSpeaking={client?.isAudioOn}>
                {client?.avatar ? (
                  <img
                    src={client.avatar}
            ...
            <ChatInnerContainer>
              ...
              <div className={'infoContainer'}>
                <p>
                  ...
                  Please use proper language and be respectful in your
                  communication.{' '}
                </p>
              </div>
              <ChatMessageContainer>
                {clientMessages?.map((eachClientMsg) => (
                  <div>
